...
class TradingSystemResults:
    ...
    market_making_paper_entries_filled: int = 0
    market_making_paper_exits_filled: int = 0
    market_making_realized_pnl: float = 0.0
...
class UnifiedAdvancedTradingSystem:
    ...
    def __init__(
        ...
        # Dynamic capital is set by async_initialize() from actual Kalshi balance.
    ...
    async def async_initialize(self):
        ...
        try:
            ...
            if self.total_capital < 10:  # Minimum $10 to trade
                self.logger.warning(f"Total capital too low: ${self.total_capital:.2f} - may limit trading")
        ...
        try:
            ...
            if cash_status['emergency_status']:
                ...
                if emergency_action.action_type == 'halt_trading':
                    ...
                    self.logger.warning(f"Need to close {emergency_action.positions_to_close} positions for cash reserves")
            
            # Enforce position limits if needed (after cash check)
            if limits_status['status'] in ['OVER_LIMIT', 'WARNING']:
                self.logger.info(f"Position limits enforcement needed: {limits_status['recommendations']}")
            ...
            self.logger.info(
                "Paper Market Making: "
                f"{results.market_making_paper_entries_filled} entries, "
                f"{results.market_making_paper_exits_filled} exits, "
                f"realized PnL ${results.market_making_realized_pnl:.2f}"
            )
    ...
    async def _execute_market_making_strategy(self, markets: List[Market]) -> Dict:
        ...
        async def _reconcile_if_paper() -> None:
            if getattr(settings.trading, "live_trading_enabled", False):
                return
            reconciliation = await self.market_maker.reconcile_persisted_paper_orders()
            default_results["paper_entries_filled"] += int(reconciliation.get("entries_filled", 0))
            default_results["paper_exits_filled"] += int(reconciliation.get("exits_filled", 0))
            default_results["realized_pnl"] += float(reconciliation.get("realized_pnl", 0.0))
        ...
        try:
            ...
            merged_results = {**default_results, **results}
    ...
    async def _execute_directional_trading_strategy(self, markets: List[Market]) -> PortfolioAllocation:
        ...
        """
        try:
            self.logger.info("Executing Directional Trading Strategy")
        ...
        try:
            ...
            for market_id, allocation_fraction in allocation.allocations.items():
                try:
                    ...
                    # Only skip if we already have the exact same market_id and side.
                    ...
                    if not can_add_position:
                        ...
                        self.logger.info(f"Position size ${initial_position_value:.2f} exceeds limits, attempting to reduce...")
                    ...
                    self.logger.info(f"Trading mode check: live_mode={live_mode} for market {opportunity.market_id}")
    ...
    def _compile_unified_results(
        ...
        try:
            ...
            return TradingSystemResults(
                ...
                market_making_paper_entries_filled=int(
                    market_making_results.get('paper_entries_filled', 0)
                ),
                market_making_paper_exits_filled=int(
                    market_making_results.get('paper_exits_filled', 0)
                ),
                market_making_realized_pnl=float(
                    market_making_results.get('realized_pnl', 0.0)
                ),
    ...
    async def _manage_risk_and_rebalance(self, results: TradingSystemResults):
        ...
        try:
            ...
            if results.portfolio_sharpe_ratio < self.config.target_sharpe_ratio * 0.5:
                self.logger.warning(f"Low Sharpe ratio: {results.portfolio_sharpe_ratio:.2f}")
            
            if results.capital_efficiency < 0.8:
                self.logger.warning(f"Low capital efficiency: {results.capital_efficiency:.1%}")
    ...
    def get_system_performance_summary(self) -> Dict:
        ...
        try:
            ...
            # Check if market_maker is initialized.
...
async def run_unified_trading_system(
    ...
    try:
        ...
        # Initialize with dynamic balance from Kalshi.
